...
namespace GEngine::Shape
{
	...
	class ParametricShape : public Geometry
	{
		//typedef Vec3f(*SurfaceFunc)(float u, float v);
		typedef std::function<Vec3f(float u, float v)> SurfaceFunc;

	public:
		ParametricShape(float uStart, float uEnd, int uResolution, float vStart, float vEnd, int vResolution, const SurfaceFunc& surfaceFunc) : Geometry()
		{
